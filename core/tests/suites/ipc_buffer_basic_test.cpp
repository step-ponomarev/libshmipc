...
TEST_CASE("buffer suggest_size - valid size calculation") {
  ...
  const IpcBufferCreateResult buffer_result =
      ipc_buffer_create(mem, ipc_buffer_suggest_size(0));
  test_utils::CHECK_OK(buffer_result);
}

TEST_CASE("buffer create - NULL memory pointer") {
  const IpcBufferCreateResult buffer_result =
      ipc_buffer_create(nullptr, ipc_buffer_suggest_size(128));
  test_utils::CHECK_ERROR(buffer_result, IPC_ERR_INVALID_ARGUMENT);
}
...
TEST_CASE("attach buffer success case") {
  ...
  const IpcBufferCreateResult create_result = ipc_buffer_create(mem, size);
