...
TEST_CASE("single writer single reader") {
    ...
    // Добавляем продюсера
    manager.add_producer(concurrent_test_utils::produce_channel, channel, 0, test_utils::DEFAULT_COUNT);
    
    // Добавляем консьюмера
    manager.add_consumer(concurrent_test_utils::consume_channel, channel, std::ref(collector), std::ref(manager.get_manager()));
...
TEST_CASE("multiple writer single reader") {
    ...
    manager.add_producer(concurrent_test_utils::produce_channel, channel, 0, test_utils::LARGE_COUNT / 3);
    manager.add_producer(concurrent_test_utils::produce_channel, channel, test_utils::LARGE_COUNT / 3, 2 * test_utils::LARGE_COUNT / 3);
    manager.add_producer(concurrent_test_utils::produce_channel, channel, 2 * test_utils::LARGE_COUNT / 3, test_utils::LARGE_COUNT);
...
TEST_CASE("multiple writer multiple reader stress") {
    ...
    manager.add_consumer(concurrent_test_utils::consume_channel, channel, std::ref(collector1), std::ref(manager.get_manager()));
    manager.add_consumer(concurrent_test_utils::consume_channel, channel, std::ref(collector2), std::ref(manager.get_manager()));
    manager.add_consumer(concurrent_test_utils::consume_channel, channel, std::ref(collector3), std::ref(manager.get_manager()));
